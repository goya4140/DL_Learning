...
class ExperimentLogger:
    ...
    用法：
        logger = ExperimentLogger(log_dir="./logs", lr_step_size=5)
        # 每个 epoch 结束后
    ...
    def __init__(self, log_dir: str = "./logs", lr_step_size: int = 5):
        self.log_dir = log_dir
        self.lr_step_size = lr_step_size
        os.makedirs(log_dir, exist_ok=True)

        # 每行：{"epoch", "train_loss", "train_acc", "val_loss", "val_acc", "lr"}
    ...
    def plot(self):
        ...
        # 学习率衰减发生的 epoch（StepLR 在第 step_size, 2*step_size... 轮后衰减）
        decay_epochs = [e for e in epochs if e % self.lr_step_size == 0 and e != epochs[-1]]

        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(13, 5))
        fig.suptitle("MLP Training Curves (MNIST)" if not _HAS_CJK else "MLP 训练曲线（MNIST）",
        ...
        ax2.annotate(f"Best: {best_val_acc:.2f}%",
                     ...
                     xytext=(best_epoch + 0.5, best_val_acc - 0.3),
